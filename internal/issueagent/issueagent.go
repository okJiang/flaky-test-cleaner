...
func renderInitialComment(in Input) string {
	...
	b.WriteString("- Evidence source: GitHub Actions job logs (excerpted; open the run/job for full context)\n")
...
func describeRuns(occ []domain.Occurrence) string {
	if len(occ) == 0 {
		return "n/a"
	}
...
func timeline(in Input, occ []domain.Occurrence) (time.Time, time.Time) {
...
func generateHypotheses(occ []domain.Occurrence) []string {
...
func reproductionIdeas(testName string, occ []domain.Occurrence) []string {
...
func suggestedNextSteps(res domain.Classification) []string {
	switch res.Class {
	case domain.ClassInfraFlake:
		return []string{
	...
	case domain.ClassLikelyRegression:
		return []string{
...
func riskNotes(res domain.Classification) []string {
	switch res.Class {
	case domain.ClassInfraFlake:
		return []string{"Noise can hide real regressions; keep infra flakes from blocking merges by routing to metrics-only pipeline."}
	case domain.ClassLikelyRegression:
		return []string{"Potential correctness regression: prioritize manual confirmation before promoting automated fixes."}
	default:
		return []string{"Flaky tests erode CI signal; each recurrence costs reruns and review time. Prioritize stabilization before enabling auto-fix."}
	}
}

func evidenceHighlights(occ []domain.Occurrence) []string {
	if len(occ) == 0 {
