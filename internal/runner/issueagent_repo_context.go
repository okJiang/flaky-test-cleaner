...
func buildIssueAgentRepoContext(ctx context.Context, ws *workspace.Manager, occ []extract.Occurrence) string {
	sha := pickHeadSHA(occ)
	...
	// Best-effort: on any workspace failure, return empty context.
	_ = ws.Ensure(ctxWS)
	...
	// Fallback: locate the test definition.
	if len(sections) == 0 {
...
func pickHeadSHA(occ []extract.Occurrence) string {
	for _, o := range occ {
...
func pickTestNameFromOccurrences(occ []extract.Occurrence) string {
	for _, o := range occ {
...
func extractGoFileLineHints(occ []extract.Occurrence) []fileLine {
...
func parseGitGrepLine(line string) (path string, lineNum int, ok bool) {
	// git grep -n returns: path:line:text
