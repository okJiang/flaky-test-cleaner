...
func TestBuildPreparationComment(t *testing.T) {
	fp := domain.FingerprintRecord{Fingerprint: "fp-123"}
...
func TestRenderFeedbackChecklist(t *testing.T) {
	fb := domain.PRFeedback{
		PRNumber: 101,
		CombinedStatus: domain.CombinedStatus{
			State: "failure",
		...
		ChangesRequested: []domain.PullRequestReview{{
			State: "CHANGES_REQUESTED",
			Body:  "please add a test",
			User:  domain.User{Login: "reviewer"},
		}},
	}
...
func TestBuildFollowUpComment(t *testing.T) {
	fp := domain.FingerprintRecord{Fingerprint: "fp-123", TestName: "TestFoo"}
	fb := domain.PRFeedback{PRNumber: 22, PRURL: "https://example.com/pr/22", HeadSHA: "abcdef123456"}
