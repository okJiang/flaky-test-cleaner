...
// Config contains all runtime options.
type Config struct {
...
func FromEnvAndFlags(args []string) (Config, error) {
	...
	fs.StringVar(&cfg.GitHubAPIBaseURL, "github-api-base-url", cfg.GitHubAPIBaseURL, "GitHub API base URL")
	...
	fs.BoolVar(&cfg.DryRun, "dry-run", cfg.DryRun, "Do not write to GitHub")
	fs.Float64Var(&cfg.ConfidenceThreshold, "confidence-threshold", cfg.ConfidenceThreshold, "Classifier threshold")
	fs.BoolVar(&cfg.TiDBEnabled, "tidb", cfg.TiDBEnabled, "Enable TiDB state store")
	fs.StringVar(&cfg.WorkspaceMirrorDir, "workspace-mirror", cfg.WorkspaceMirrorDir, "Path to bare mirror")
	fs.StringVar(&cfg.WorkspaceWorktreesDir, "workspace-dir", cfg.WorkspaceWorktreesDir, "Path holding worktrees")
	fs.IntVar(&cfg.WorkspaceMaxWorktrees, "workspace-max", cfg.WorkspaceMaxWorktrees, "Maximum concurrent worktrees")
	fs.BoolVar(&cfg.RunOnce, "once", cfg.RunOnce, "Run one cycle and exit")
	fs.DurationVar(&cfg.DiscoveryInterval, "discovery-interval", cfg.DiscoveryInterval, "Discovery interval (0 disables)")
	fs.DurationVar(&cfg.InteractionInterval, "interaction-interval", cfg.InteractionInterval, "Interaction interval (0 disables)")
	fs.StringVar(&cfg.CopilotModel, "copilot-model", cfg.CopilotModel, "Copilot model ID")
	fs.DurationVar(&cfg.CopilotTimeout, "copilot-timeout", cfg.CopilotTimeout, "Copilot timeout per request")
	fs.StringVar(&cfg.CopilotLogLevel, "copilot-log-level", cfg.CopilotLogLevel, "Copilot log level")
